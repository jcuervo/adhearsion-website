...
class SandboxUser < ActiveRecord::Base
  ...
  validates_format_of :email, :with => /^[\w\._+%-]+@[\w\.-]+\.[a-zA-Z]{2,6}$/, :message => "not in the proper username@domain.com format"

  validates_format_of :username, :with => /^[a-z][\w_]{3,}$/i, :message => "must be at least four characters and begin with letter"
